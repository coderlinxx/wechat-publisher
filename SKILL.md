...
└────────────────────────────────────────────────────┘
  ↓
转换为微信 HTML（纯内联样式，用微信 CDN URL）
  ↓
上传文章内联图片/视频到微信 CDN
  ↓
创建草稿（微信 API，保存到微信公众号后台）
  ↓
【🔍 内容审核关键步骤】
  ↓
  ├─ 本地 CLI：
  │   ├─ 写入 Markdown → /tmp/wechat-draft-{timestamp}.md
  │   ├─ 完整显示内容（含 frontmatter 和图片 URL）
  │   └─ 提示用户确认（y/n）：「确认发布到网站数据库？」
  │
  └─ OpenClaw 机器人（飞书/QQ）：
      ├─ 返回审核结果给用户（Markdown 内容）
      ├─ 用户在手机上查看内容质量
      └─ 用户回复确认发布
  ↓
用户确认（✅）
...
│    网站前端实时生效                                 │
│                                                     │
│ ② 本地备份到 content/publisher/                   │
│    （如有 portfolio-v2）                           │
└────────────────────────────────────────────────────┘
  ↓
完成！🎉
  - 🔗 微信：草稿已在微信公众号后台（Media ID）
  - 🖼️ 网站：内容同步到网站数据库（Convex）
  - 💾 备份：本地 Markdown 已保存
```

### 审核工作流详解

**目的**：在 Convex 同步前审核内容质量，确保无误再发布到网站

**本地 CLI 模式**：
```bash
# 1. 用户运行
node scripts/publish.mjs --title "..." --content "..." --author "..."

# 2. 脚本创建草稿到微信，然后显示完整 Markdown
# Media ID: 12345678
# 📝 内容已准备好审核
...
OpenClaw → 创建微信草稿
         ↓
OpenClaw → 返回审核卡片（显示 Markdown）
     💬「✅ 内容已生成并推送到微信草稿箱。请确认内容质量，确认后将发布到网站数据库」
         ↓
用户 → 在手机上查看 Markdown 内容（无需编辑）
         ↓
用户 → 回复「✅ 确认发布」
         ↓
OpenClaw → 执行 Convex 同步和本地备份
         ↓
OpenClaw → 返回完成消息（显示 Media ID 和 URL）
```

**核心优势**：
- ✅ 无需创建多次 - 草稿只创建一次，用户审核后直接使用
- ✅ 适合移动端 - 手机上清晰查看，不用编辑
- ✅ 避免重复工作 - 审核通过后不再重新生成封面/转换格式
- ✅ 保证内容一致 - 审核的是最终发布版本的 Markdown
